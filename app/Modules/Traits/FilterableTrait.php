<?php namespace App\Modules\Traits;
...
trait FilterableTrait
{
     ...
     * @return $this
     */
    public function setQuerystring(array $str = [], $append = true, $default = true)
    {
        if (is_null($this->filterable)) {
            $this->resetFilterableOptions();
        }
     ...
     */
    public function scopeFilterColumns($query, $columns = [], $validate = false)
    {
            ...
            foreach ($this->filterable['filters'] as $k => $v) {
                ...
                if (is_array($v['val'])) {
                    if (isset($v['val']['start']) && isset($v['val']['end'])) {
                        // BETWEEN a AND b
                        $query->whereBetween($k, [$v['val']['start'], $v['val']['end']]);
                    }
                    else {
                        // a = b OR c = d OR...
